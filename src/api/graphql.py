...
async def make_grpc_call(
    ...
    *request_message: Message,
    ...
    :param request_message: The request message to send (e.g., ``product_service_models.ModelId(id=id)``).
...
class Query:
    ...
    async def get_model(self, id: int) -> Model:
        return await make_grpc_call(
            product_service_stub.GetModel,
            Model,
            product_service_models.ModelId(id=id),
        )
...
class Mutation:
    ...
    async def create_model(self, model: ModelBase) -> Model:
        return await make_grpc_call(
            product_service_stub.CreateModel,
            Model,
            model.to_grpc(),
        )
